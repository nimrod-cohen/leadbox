	window.lpManager = {
		...
		init : function(options)
		{
			...
			//cleanup callbacks and clear the array
			for(var i=0;i<this.resizeCallbacks.length;i++) 
				jQuery(window).off("resize",this.resizeCallbacks[i]);
			...
			jQuery("div[leadbox-form]").each(function()
			{
				...
				self.resizeObjects(host,frm);

				var fnc = function(){self.resizeObjects(host,frm);}
				self.resizeCallbacks.push(fnc);
		...
		{
			return jQuery("#"+fieldId).closest("div[leadbox-form]").attr('id');
		},
		...
		render : function(host,form)
		{
			...
			if(this.langTable[this.options.language]["tag_line"].length > 0)
				formText += "<h3 class='lp-title'>"+$("<div/>").html(this.langTable[this.options.language]["tag_line"]).text()+"</h3>"
			...
			host.append("<style>\n"+
				...
				"form.lp-container h3.lp-title {padding: 10px;margin:0;text-align: center;color:"+this.options.color+"} " +
		...
		//in case the submit button is not horizontally inlined with the last form field, we move it below it.
		switchSubmitDirection : function(host,form)
		{
			var fld = jQuery("#"+form[form.length-1].id);
	   		var btn = jQuery("#"+form[0].id);
			if(!fld.length || !btn.length) //ugly hack, don't know why sometimes this is null
				return;
			var row = btn.closest(".lp-row");
			var hostLeft = host.offset().left;

	  		row.css("left","");
			if(fld.offset().top != btn.offset().top )
		    	row.css("left",fld.offset().left - btn.offset().left);
		},

		resizeObjects : function(host,form)
		{
			...
			var self = this;
			if(this.options.submit.oppositeDirection && this.options.layout != 'vertical')
				this.switchSubmitDirection(host,form);
		},
		...
		saveDetails : function(e)
		{
			...
			var hostId = lpManager.getHostId(jQuery(e.target).attr("id"));
		...
		doSubmit : function(hostId)
		{
			...
			for(var i =0; i<form.length;i++)
			{
				...
				if(field.id == hostId+'_submit_button')
					...
					data[field.name] = jQuery("#"+field.id).is(":checked");
				else
					data[field.name] = jQuery("#"+field.id).val();
			}
